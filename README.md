[code_artifact (4).html](https://github.com/user-attachments/files/31537378/code_artifact.4.html)
<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
...
<body class="bg-slate-900 text-slate-800 font-sans min-h-screen flex flex-col overflow-x-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2 bg-slate-900/60 p-1 rounded-xl border border-slate-700/60">
                <button id="btnTabDisplay" onclick="switchTab('display')" class="px-4 py-2 rounded-lg text-sm font-medium transition flex items-center space-x-2 bg-sky-500 text-slate-950 font-bold shadow-md">
                    <i class="fa-solid fa-[#faad14] fa-tv"></i>
                    <span>動態展示場景</span>
                </button>
            ...
            <div class="flex items-center space-x-3">
                <span id="syncStatus" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-sky-500/10 text-sky-400 border border-sky-500/20">
                    <span class="w-2 h-2 mr-1.5 rounded-full bg-sky-400 animate-pulse"></span>
                    雲端同步中
                </span>
    ...
    <main class="flex-1 relative overflow-hidden flex flex-col">
        ...
        <div id="panelDisplay" class="absolute inset-0 w-full h-full flex flex-col bg-slate-950">
            ...
            <div id="newArtworkBanner" class="hidden absolute top-4 left-1/2 -translate-x-1/2 z-30 bg-gradient-to-r from-sky-500 via-blue-500 to-indigo-500 text-white font-bold px-6 py-2.5 rounded-full shadow-2xl flex items-center space-x-3 border border-white/30 animate-bounce">
                <i class="fa-solid fa-sparkles text-xl text-sky-200"></i>
            ...
            <div class="max-w-7xl mx-auto space-y-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-5 bg-slate-800/60 rounded-3xl p-6 border border-slate-700/60 flex flex-col space-y-5 shadow-xl">
                        ...
                        <div id="dropzone" class="border-2 border-dashed border-slate-600 hover:border-sky-500 transition rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer bg-slate-900/40 relative">
                            ...
                            <div id="uploadPlaceholder" class="space-y-3">
                                ...
                                <p class="text-xs text-slate-400">支援 JPG, PNG 格式，建議以白色畫紙為背景</p>
                            </div>
                        </div>
                        ...
                        <div id="editorStudio" class="hidden space-y-4">
                            ...
                            <div class="relative bg-checkerboard rounded-2xl overflow-hidden border border-slate-700 flex items-center justify-center min-h-[260px] max-h-[380px]">
                                ...
                                <div class="absolute bottom-2 left-2 bg-slate-900/80 backdrop-blur-md px-2.5 py-1 rounded-md text-[11px] text-slate-300">
                                    提示: 點擊圖片點選要消除的背景顏色
                                </div>
                            </div>
                            ...
                            <div class="space-y-3 bg-slate-900/60 p-4 rounded-xl border border-slate-700/60">
                                ...
                                <div class="pt-2 border-t border-slate-800 flex items-center justify-between">
                                    <label class="flex items-center space-x-2 cursor-pointer text-xs text-slate-300">
                                        ...
                                        <span class="font-medium text-sky-300">智慧連通去背（保護人物內部白色/亮色）</span>
                                    </label>
                                </div>
                            </div>
    ...
    <script type="module">
        ...
        import { getFirestore, collection, addDoc, deleteDoc, doc, updateDoc, setDoc, onSnapshot } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
        ...
        const state = {
            ...
            activeDisplayedIds: [],
            minDisplayCount: 10,
            rotationInterval: 180, // 180 seconds (3 minutes)
            timerCountdown: 180,
            timerId: null,
        ...
        // Save Background to Firestore Cloud for Cross-Device Realtime Sync
        ...
        window.handleBgUpload = function(event) {
            ...
            showToast("正在處理並同步背景照片...", "info");

            const reader = new FileReader();
            reader.onload = function(e) {
                const img = new Image();
                img.onload = function() {
                    // Resize custom background to max 1920x1080 to fit within Firestore doc limit
                    ...
                    showToast("新校園背景已成功同步至所有展示裝置！", "success");
                };
        ...
        async function syncBgToCloud(bgUrl) {
            ...
            try {
                ...
                console.error("Failed to sync background to Firestore:", err);
            }
        }
        ...
        function applyCampusBg(bgUrl) {
            ...
            if (selectEl) {
                // Check if background URL corresponds to one of the select options
                let matched = false;
        ...
        function rotateArtworks() {
            ...
            // Shuffle artworks array slightly to show next batch
        ...
        async function initFirebase() {
            try {
                ...
                console.log("Firebase Auth Connected:", state.user.uid);
                listenToArtworks();
                listenToSettings();
            } catch (err) {
                console.warn("Firebase Auth Note (Running in standalone preview mode):", err);
                initLocalFallback();
            }
        }

        // Firestore Listener for Global Background Settings
        ...
        // Firestore Real-time Listener (Rule 1 & Rule 2 compliance)
        function listenToArtworks() {
            if (!state.user || !state.db) return;

            // Public path collection for multi-device sync
            ...
            onSnapshot(artworksCollection, (snapshot) => {
                ...
                // Sort in memory by createdAt descending (Newest first) - Rule 2 compliance
        ...
        function initLocalFallback() {
            showToast("以本機模式運行中", "info");
            renderGalleryGrid();
            refreshDisplayCharacters();
        }

        // Image Upload & Canvas Chroma Key Background Removal logic
        window.handleFileSelect = function(event) {
            ...
            reader.onload = function(e) {
                ...
                img.onload = function() {
                    ...
                    // Auto sample top-left pixel as background default color
        ...
        // Eyedropper / Color Picker on Canvas Click
        ...
        window.autoDetectBackground = function() {
            state.editor.targetRgb = { r: 245, g: 245, b: 245 }; // Target white/light gray paper
        ...
        // Core Chroma Key / Background Removal Algorithm
        function processBackgroundRemoval() {
            ...
            if (useFloodFill) {
                // Smart Outer-Edge Flood Fill Algorithm: Only erase outer paper background
                ...
                // Add outer border pixels to starting queue
                ...
                while (qHead < qTail) {
                    ...
                    // Turn outer background transparent
                    data[pos * 4 + 3] = 0;

                    // 4-directional neighborhood search
                    const neighbors = [
                        px > 0 ? pos - 1 : -1,
                        px < width - 1 ? pos + 1 : -1,
                        py > 0 ? pos - width : -1,
                ...
                // Classic global chroma key removal
        ...
        // Save Artwork to Firestore Cloud
        window.saveArtworkToCloud = async function() {
            ...
            // Convert processed transparent canvas to base64
            const processedBase64 = processCanvas.toDataURL('image/png', 0.85);
            ...
            try {
                if (state.db && state.user) {
                    ...
                    // Fallback local memory addition
                ...
                showToast("作品已成功發佈至校園展示舞台！", "success");
                resetUploadForm();
                switchTab('display');
            } catch (e) {
        ...
        function renderGalleryGrid() {
            ...
            grid.innerHTML = state.artworks.map((item) => `
                <div class="bg-slate-800 rounded-2xl p-3 border ${item.active ? 'border-slate-700' : 'border-red-900/40 opacity-60'} flex flex-col justify-between space-y-3 group shadow-md hover:border-slate-600 transition">
                    ...
                    <!-- Editable Artwork Name and Scale Controls -->
                    <div class="space-y-2">
                        <div>
        ...
        window.updateArtworkName = async function(id, newName) {
            ...
            try {
                if (state.db && state.user && !id.startsWith('local_')) {
                ...
                showToast("作品名稱已同步更新", "success");
            } catch (e) {
                console.error("Update name failed:", e);
                showToast("更新名稱失敗", "error");
            }
        };
        ...
        window.updateArtworkScale = async function(id, newScale) {
            try {
                ...
                showToast("作品尺寸已同步更新", "success");
            } catch (e) {
                console.error("Update scale failed:", e);
                showToast("更新尺寸失敗", "error");
            }
        };
        ...
        window.toggleArtworkActive = async function(id, newActive) {
            try {
                ...
                console.error("Update active status failed:", e);
            }
        };
        ...
        // Campus Interactive Animation Logic
        function refreshDisplayCharacters() {
            ...
            // Select display set based on minDisplayCount (minimum 10)
            const countToDisplay = Math.max(state.minDisplayCount, Math.min(availableArtworks.length, 12));
            
            // Priority selection: Newer artworks + rotated slice of existing ones
            ...
            selectedSet.forEach((item, index) => {
                ...
                // Initial physics vectors (spread across campus screen)
                const posX = Math.random() * (containerW - charSize - 40) + 20;
                const posY = Math.random() * (containerH - charSize - 120) + 60;
                ...
                // Bind Touch and Mouse Drag interactions
        ...
        // Animation Loop for Moving Characters around Campus
        function animateCharacters() {
            ...
            if (container) {
                ...
                state.charactersPhysics.forEach((phys) => {
                    ...
                    // Bounce off horizontal screen walls
                    if (phys.x <= 10) {
                        phys.x = 10;
                    ...
                    // Bounce off vertical screen walls
                    if (phys.y <= 40) {
                        phys.y = 40;
        ...
        function setupDragEvents(el, physObj) {
            ...
            const onEnd = () => {
                ...
                // If user tapped/clicked without dragging far (< 10px), trigger Jump feature!
                if (totalMoved < 10) {
                    triggerCharacterJump(el, physObj);
                }

                // Fallback gentle float if dropped stationary
            ...
            // Touch Event Listeners (Mobile / iPad / Touch Screens)
        ...
        // Trigger Jump Animation & Visual Effects when character is clicked/tapped
        function triggerCharacterJump(el, physObj) {
            ...
            // Give upward physics bounce boost
        ...
        // Web Audio API Q-bounce sound synthesizer
        function playBoingSound() {
            try {
                ...
                // Ignore audio context block if user has not interacted with browser yet
            }
        }
