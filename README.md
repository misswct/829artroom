[Uploading code_artifact (7).html…]()
<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    ...
    <script type="module">
        ...
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'school-openday-default';
        let db = null;
        let auth = null;
        ...
        window.initCloudSync = async function() {
            if (!db || !auth) {
                ...
                updateSyncStatusUI(false, "離線預覽模式");
                return;
            }
            ...
            try {
                ...
                updateSyncStatusUI(true, "雲端實時同步中");

                // Listen to Cloud Artworks Collection
                const colRef = collection(db, 'artifacts', appId, 'public', 'data', 'artworks');
                onSnapshot(colRef, (snapshot) => {
                    const cloudArtworks = [];
                    snapshot.forEach(docSnap => {
                        cloudArtworks.push(docSnap.data());
                    });

                    cloudArtworks.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));

                    const prevCount = state.artworks.length;
                    const isNew = cloudArtworks.length > prevCount && prevCount > 0;

                    state.artworks = cloudArtworks;
                    saveLocalArtworks();
                    renderGalleryGrid();
                    refreshDisplayCharacters();

                    if (isNew && cloudArtworks.length > 0) {
                        const newest = cloudArtworks[0];
                        showBanner(`🎉 「${newest.name || '小朋友'}」的作品剛剛加入了校園舞台！`);
                        playFanfareSound();
                    }
                }, (error) => {
                    console.error("Firestore error:", error);
                    updateSyncStatusUI(false, "連線異常");
                });

                // Listen to Cloud Background Settings
                const settingsRef = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'global');
                onSnapshot(settingsRef, (docSnap) => {
                    if (docSnap.exists() && docSnap.data().bgUrl) {
                        state.currentBgUrl = docSnap.data().bgUrl;
                        applyCampusBg(docSnap.data().bgUrl);
                    }
                }, (err) => console.warn("Settings sync error:", err));

            } catch (err) {
                console.error("Cloud Auth Failed:", err);
                updateSyncStatusUI(false, "連線失敗");
            }
        };

        window.syncSaveArtwork = async function(artworkDoc) {
            if (db && auth?.currentUser) {
                try {
                    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'artworks', artworkDoc.id), artworkDoc);
                } catch(e) { console.error("Cloud save error:", e); }
            }
        };

        window.syncDeleteArtwork = async function(id) {
            if (db && auth?.currentUser) {
                try {
        ...
        window.syncUpdateBg = async function(bgUrl) {
            if (db && auth?.currentUser) {
                try {
                    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'global'), { bgUrl });
                } catch(e) { console.error("Cloud bg update error:", e); }
            }
        };
    </script>
...
<body class="bg-slate-900 text-slate-800 font-sans min-h-screen flex flex-col overflow-x-hidden">
    ...
    <header class="bg-slate-800 border-b border-slate-700 text-white z-30 flex-shrink-0 sticky top-0 shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <!-- Right Actions -->
            <div class="flex items-center space-x-3">
                <div id="netStatusBtn" class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                    <span id="syncDot" class="w-2 h-2 mr-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                    <span id="syncText">雲端實時同步中</span>
                </div>
    ...
    <script>
        ...
        // Custom UI Status Update
        function updateSyncStatusUI(isOnline, textMsg) {
            const syncText = document.getElementById('syncText');
            const syncDot = document.getElementById('syncDot');
            if (syncText) syncText.innerText = textMsg || (isOnline ? "雲端實時同步中" : "離線模式");
            if (syncDot) {
                syncDot.className = isOnline ? "w-2 h-2 mr-1.5 rounded-full bg-emerald-400 animate-pulse" : "w-2 h-2 mr-1.5 rounded-full bg-slate-400";
            }
        }
        ...
        window.changeCampusBackground = function(bgUrl) {
            ...
            if (window.syncUpdateBg) window.syncUpdateBg(bgUrl);
        };
        ...
        window.handleBgUpload = function(event) {
            ...
            reader.onload = function(e) {
                ...
                img.onload = function() {
                    ...
                    if (window.syncUpdateBg) window.syncUpdateBg(compressedBase64);
        ...
        window.saveArtworkToCloud = async function() {
            ...
            if (window.syncSaveArtwork) {
                await window.syncSaveArtwork(newDoc);
            }
        ...
        window.updateArtworkName = function(id, newName) {
            ...
            if (found) {
                ...
                renderGalleryGrid();
                refreshDisplayCharacters();
                if (window.syncSaveArtwork) window.syncSaveArtwork(found);
            }
        ...
        window.deleteArtwork = function(id) {
            ...
            renderGalleryGrid();
            refreshDisplayCharacters();
            if (window.syncDeleteArtwork) window.syncDeleteArtwork(id);
